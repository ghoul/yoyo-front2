...
function Category() {
  ...
  useEffect(() => {
    if (categoryName) {
      axios
        .get(`https://jellyfish-app-lfx7p.ondigitalocean.app/service2/category/type/${categoryName}/`, {
          headers: {
            'Authorization': `${token}`,
  ...
  useEffect(() => {
    if (categoryID) {
      axios
        .get(`https://jellyfish-app-lfx7p.ondigitalocean.app/service2/ategories/${categoryID}/tricks/`, {
          headers: {
            'Authorization': `${token}`,
            'Content-Type': 'application/json',
          },
